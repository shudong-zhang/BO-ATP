...
def load_cifar10_test_data(test_batch_size=1):
    # CIFAR10 Dataset
    test_dataset = dsets.CIFAR10('/dataset', download=True, train=False, transform=transforms.ToTensor())
    test_loader = torch.utils.data.DataLoader(dataset=test_dataset, batch_size=test_batch_size, shuffle=True)

    return test_loader


def load_imagenet_test_data(test_batch_size=1, folder='/dataset/imagenet/Sample_1000'):
